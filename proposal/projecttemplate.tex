...
We train our bot using fictitous self-play (managed by A and B), and utilize abstraction heuristics (managed by C and D). Our method was able to consistently beat several baseline agents, including Libratus \cite{brown2019poker} (managed by E).
\subsection{Related Work}\label{sec:related}
This subsection should contain all the references to related works, methods, approaches and so on. This part should not be longer than two paragraphs. You should also cite as you go. 
...
When citing, do not treat numbered citations as names. We cannot say: ``\cite{Caragiannis2016MNW} show that...''; rather use ``\citet{Caragiannis2016MNW} show that''. You can use \citeauthor{Caragiannis2016MNW} for just the authors' name if needed. Usually this is appropriate if used repeatedly e.g. \citet{bhaskar2021envyfreeness} describe A and B. However, \citeauthor{bhaskar2021envyfreeness} do not describe C and D. 
